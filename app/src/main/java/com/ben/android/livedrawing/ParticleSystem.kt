...
class ParticleSystem {
    ...
    fun initParticles(numParticles:Int){
        ...
        for (i in 0 until numParticles) { //until no inlucye el elemento final
            var angle: Double = random.nextInt(360).toDouble() //se genera un número aletorio entre 0 y 359
            angle *= (3.14 / 180)  //degree to radian
            ...
            direction = PointF(Math.cos(angle).toFloat() * speed,Math.sin(angle).toFloat() * speed)
